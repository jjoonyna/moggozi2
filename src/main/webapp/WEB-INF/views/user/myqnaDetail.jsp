					...
					<div class="cards">
						<c:choose>
								...
								<table class="navClass">
									<tr class="classLeft">
										<td class="spanNicks"><span class="spanNick1">작성자:
												${notice.usernick}</span> <span class="spanNick4">등록일: <fmt:formatDate
													value="${notice.notiDate}" pattern="yyyy.MM.dd HH:mm" /></span></td>
									</tr>
									<tr class="classLeft">
										<td class="taContent">
							...
							<c:otherwise>
								<table class="navClass">
									<tr class="classLeft">
										<td class="taContent">
											<!-- 수정된 부분 --> <textarea id="noticeContent" readonly>문의 정보를 찾을 수 없습니다.</textarea>
										</td>
									</tr>
								</table>
							</c:otherwise>
						...
						<c:choose>
							<c:when test="${not empty replies}">
								...
								<c:forEach items="${replies}" var="reply">
									<table class="navClass">
										<tr class="classLeft">
											<td class="spanNicks"><span class="spanNick1">작성자:
													${reply.usernick}</span> <span
												style="position: relative; right: -160px;">등록일: <fmt:formatDate
														value="${reply.replyDate}" pattern="yyyy.MM.dd HH:mm" /></span>
											</td>
										</tr>
										<tr class="classLeft">
											<td class="taContent">
												<!-- 수정된 부분 --> <textarea readonly>${reply.replyContent}</textarea>
											</td>
										</tr>
									</table>
								</c:forEach>
							...
							<c:otherwise>
								<table class="navClass">
									<tr class="classLeft">
										<td class="taContent">
											<!-- 수정된 부분 --> <textarea id="noticeContent" readonly>답변이 없습니다.</textarea>
										</td>
									</tr>
								</table>
							</c:otherwise>
						...
						<div class="btn">
							<input type="button" value="목록" class="listBtn"
								onclick="window.location.href='myqnaList?usernick=${usernick}'">
						</div>
					</div>
				</section>
			</nav>
